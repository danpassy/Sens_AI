...
class AcquisitionUI(tk.Tk):
    def __init__(self) -> None:
        ...
        self.tree_item_to_path: dict[str, Path] = {}
    ...
    def _center_window(self) -> None:
        ...
        x_pos = max((self.winfo_screenwidth() - width) // 2, 0)
        y_pos = max((self.winfo_screenheight() - height) // 2, 0)
    ...
    def _build_layout(self) -> None:
        ...
        right_card.rowconfigure(2, weight=3)
        right_card.rowconfigure(4, weight=2)
        ...
        history_frame = ttk.Frame(right_card, style="Card.TFrame")
        ...
        self.tree.heading("file", text="Fichiers")
        ...
        ttk.Separator(right_card, orient=tk.HORIZONTAL).grid(row=3, column=0, sticky="ew", pady=(10, 10))

        preview_frame = ttk.Frame(right_card, style="Card.TFrame")
        preview_frame.grid(row=4, column=0, sticky="nsew")
        preview_frame.columnconfigure(0, weight=1)
        preview_frame.rowconfigure(1, weight=1)

        self.preview_title_var = tk.StringVar(value="Aperçu du fichier")
        ttk.Label(preview_frame, textvariable=self.preview_title_var, style="Field.TLabel").grid(
        ...
        self._set_preview_content("Sélectionnez un fichier dans l'historique pour afficher son aperçu.")
    ...
    def _acquire_and_save(self, config: AcquisitionConfig) -> tuple[Path, int]:
        ...
        json_obj = {
            ...
            "payload": {
                "device_name": "00:10:57:79:96:15",
                "device_type": "DATA_FORWARDER",
                "interval_ms": 200,
                "sensors": [
                    {"name": "acc_x", "units": "m/s2"},
                    {"name": "acc_y", "units": "m/s2"},
                    {"name": "acc_z", "units": "m/s2"},
                    {"name": "gyr_x", "units": "N/A"},
                    {"name": "gyr_y", "units": "N/A"},
                    {"name": "gyr_z", "units": "N/A"},
                    {"name": "htr", "units": "N/A"},
                    {"name": "spo", "units": "N/A"},
                ],
                "values": values,
    ...
    def _load_history(self) -> None:
        ...
        if not DATA_ROOT.exists():
            self.preview_title_var.set("Aperçu du fichier")
            self._set_preview_content("Aucune acquisition disponible.")
            return
        ...
        for file_path in files:
            ...
            self.tree_item_to_path[row_id] = file_path
    ...
    def _on_tree_select(self, _event: tk.Event | None = None) -> None:
        ...
        item_id = selection[0]
        file_path = self.tree_item_to_path.get(item_id)
        if file_path is None:
            self.preview_title_var.set("Aperçu du fichier")
            self._set_preview_content("Sélectionnez un fichier (ligne bleue) pour afficher son contenu.")
            return

        self.preview_title_var.set(f"Aperçu: {file_path.name}")
        try:
            preview = self._read_file_preview(file_path)
        except Exception as exc:
            preview = f"Impossible de lire le fichier:\n{file_path}\n\nDétail: {exc}"
        self._set_preview_content(preview)

    def _read_file_preview(self, file_path: Path, max_lines: int = 120) -> str:
        lines: list[str] = []
        truncated = False
        ...
        preview = "\n".join(lines)
        if truncated:
            preview += "\n\n... aperçu tronqué (120 lignes max)"
        return preview

    def _set_preview_content(self, text: str) -> None:
